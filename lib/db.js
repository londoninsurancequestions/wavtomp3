...
export async function findUserById(id) {
  return dbGet(
    'SELECT id, email, stripe_customer_id, unlocked_at, created_at FROM users WHERE id = ?',
    [id]
  );
}
...
export async function findUserByStripeCustomerId(customerId) {
  return dbGet(
    'SELECT id, email, stripe_customer_id, unlocked_at, created_at FROM users WHERE stripe_customer_id = ?',
    [customerId]
  );
}

export async function markUserUnlocked(userId) {
  const unlockedAt = new Date().toISOString();
  await dbRun('UPDATE users SET unlocked_at = ? WHERE id = ?', [unlockedAt, userId]);
}
